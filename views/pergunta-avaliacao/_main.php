...
?>
    <div class="product-form">
        ...
        echo Html::button('(+) Pergunta', [
            'id' => 'product-new-pergunta-avaliacao-button',
            'class' => 'pull-right btn btn-primary btn-s'
        ]);

        // parcel table
        $parcel = new \app\models\PerguntaAvaliacao();
        ...
        // new parcel fields
        echo '<tr id="product-new-pergunta-avaliacao-block"  style="display: none;">';
        echo $this->render('_multi', [
            'key' => '__id__',
            'form' => $form,
            'parcel' => $parcel,
...
$mjs = "var parcel_k =  0;
        $('#product-new-pergunta-avaliacao-button').on('click', function () {
            
            $('#product-parcels').find('tbody')
                .append('<tr>' + $('#product-new-pergunta-avaliacao-block').html().replace(/__id__/g, parcel_k) + '</tr>');

            parcel_k += 1;
            // remove parcel button
            $(document).on('click', '.product-remove-pergunta-avaliacao-button', function () {
