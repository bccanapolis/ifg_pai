...
<?php 
    $gridColumn = [
        ...
        [
                'attribute' => 'id_pergunta_avaliacao',
                'label' => 'Id Pergunta Avaliacao',
                'value' => function($model){
                    if($model->id_pergunta_avaliacao!=NULL){
                        return $model->perguntaAvaliacao->{\app\models\PerguntaAvaliacao::representingColumn()};
                    }else{
                        return NULL;
                    }
                },
                'filterType' => GridView::FILTER_SELECT2,
                'filter' => \yii\helpers\ArrayHelper::map(\app\models\PerguntaAvaliacao::find()->asArray()->orderBy(\app\models\PerguntaAvaliacao::representingColumn())->all(), 'id', \app\models\PerguntaAvaliacao::representingColumn()),
                'filterWidgetOptions' => [
                    'pluginOptions' => ['allowClear' => true],
                ],
